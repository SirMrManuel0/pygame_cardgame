...
for players in range(min_player, max_player + 1):
    for cards in range(1, max_cards + 1):
        ...
        avg_reward, std_reward, moving_avg_reward, avg_loss, avg_entropy = train(
            Difficulties.EASY,
            cards=cards,
            players=players,
            episodes=episodes,
...
for players in range(2, max_player + 1):
    for cards in range(1, max_cards + 1):
        ...
        to_print += f"{rewards_color(std_reward)}Std Reward: {std_reward:.4f}{Style.RESET_ALL} | "\
                    f"Moving Avg: {moving_avg:.4f} | "\
                    f"{loss_color(avg_loss)}Loss: {avg_loss:.4f}{Style.RESET_ALL} | "\
